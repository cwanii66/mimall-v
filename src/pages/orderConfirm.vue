        ...
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{'checked': index === checkedId}"
                v-for="(item, index) in list"
                :key="item.id"
                @click="checkedId = index"
              >
...
export default {
    ...
    data() {
        return {
            ...
            showEditModal: false, // 是否显示新增活着编辑弹框
            checkedId: 0, // 当前选中收货地址索引
        }
    },
    ...
    methods: {
        ...
        getCartList() {
            this.axios.get('/carts').then((res) => {
                ...
                this.cartTotalPrice = res.cartTotalPrice; // 商品总金额（我们已经选中的）
        ...
        orderSubmit() {
          ...
          this.axios.post('/orders', {   // 订单创建成功后商品数据就会被清空
            shippingId: item.id
          }).then((res) => {
            this.$router.push({
              path: '/order/pay',
